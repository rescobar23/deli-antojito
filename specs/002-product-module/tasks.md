...
**Tests**: Tests are mandatory for this behavior change per project constitution. Write failing tests before implementation for each story.
...
**Independent Test**: Open the product module with empty and populated local data; verify list rows, placeholders, prices, active status, and summary counts render correctly.
...
**Goal**: User can navigate from the list to add a product, validate fields and image, save, and return to the list with the new product visible.

**Independent Test**: From the product list, tap add, enter valid product data, save, and verify the product appears in the list; invalid inputs show messages and preserve form data.
...
- **User Story 2 (Phase 4)**: Depends on Foundational and uses US1 navigation/list route for end-to-end flow.
...
- T003, T004, T005, and T007 can run in parallel after T001-T002 decisions.
- T008, T011, T012, and T013 can run in parallel during Foundational work.
...
- Domain use case tasks for a story can run in parallel with UI shell tasks when they do not touch the same file.

## Parallel Example: User Story 1

```text
Task: T020 [US1] Add product validation unit tests
Task: T021 [US1] Add DAO list and summary fixture tests
Task: T022 [US1] Add repository observe-products tests
Task: T023 [US1] Add ProductViewModel state tests
Task: T024 [US1] Add product list Compose UI tests
```
...
2. Add US2 product creation and validation.
...
- `[P]` means task can run in parallel only when earlier dependencies are complete.
